...
- Gated endpoints include: `POST /api/merge-pr`, `POST /api/merge-pr/reject`, `POST /api/reset`, `POST /api/state/unmask`, `POST /api/resource`, `POST /api/alerts/config`.
...
This project detects Infrastructure/Resource drift ONLY: Terraform-managed AWS resources versus live state via `terraform plan` and AWS SDK inspection. Explicitly out of scope: OS/config drift, application/code drift, environmental drift, data/schema drift, and operational/lifecycle drift. Do not add detection logic for these categories — they require fundamentally different data sources and tools, not extensions of this pipeline.
