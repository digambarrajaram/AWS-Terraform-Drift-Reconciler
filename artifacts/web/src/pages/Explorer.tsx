...
function EventDetail({ e }: { e: DriftEvent }) {
  return (
    <div className="space-y-6 text-sm">
      ...
      {e.fields_changed && e.fields_changed.length > 0 && (
        <section className="space-y-2">
...
function DriftCard({
  ...
  const hasFields  = !!event.fields_changed && event.fields_changed.length > 0;
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          ...
          {hasFields && (
            <span className="text-[11px] text-muted-foreground">
              {event.fields_changed!.length} field{event.fields_changed!.length !== 1 ? 's' : ''} changed
            </span>
          )}
        </div>
      </div>
...
export default function Explorer() {
  ...
  useEffect(() => {
    ...
    filters.dateFrom, filters.dateTo, sort,
  ]);
