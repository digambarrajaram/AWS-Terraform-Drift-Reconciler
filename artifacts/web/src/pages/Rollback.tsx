      ...
      <table className="w-full text-xs min-w-[640px]">
        ...
        <tbody className="divide-y divide-border">
          {diff.map((row, i) => {
            ...
            return (
              <tr key={i} className={stale ? 'bg-amber-50/40 dark:bg-amber-900/10' : ''}>
...
export default function Rollback() {
  ...
  useEffect(() => {
    ...
    if (run.status === 'complete') {
      ...
      setCtx((prev) => prev ? { ...prev, failed: true, failedPhase: 'preview' } : prev);
      setPhase('preview_done');
    }
  ...
  useEffect(() => {
    ...
    if (run.status === 'complete') {
      ...
      setCtx((prev) => prev ? { ...prev, failed: true, failedPhase: 'execute' } : prev);
      setPhase('execute_done');
      toast.error('Rollback execution failed', {
        description: 'Check the log above for details.',
      });
    }
    ...
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      {phase !== 'idle' && ctx && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          ...
          {/* Execute failed */}
          {phase === 'execute_done' && ctx.failed && ctx.failedPhase === 'execute' && (
            <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3 flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Execution failed. Review the log above.</span>
            </div>
          )}
        </div>
      )}
